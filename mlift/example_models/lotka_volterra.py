...
def sample_initial_states(
    ...
    peak_times_obs_0 = utils.calculate_peak_times(
        x_obs[:, 0], data["t_seq"], smoothing_window_width, peak_value_threshold
    )
    ...
    while len(init_states) < num_chain and num_tries < max_init_tries:
        ...
        if not onp.all(onp.isfinite(x)) or not onp.all(x > 0):
            num_tries += 1
            continue
        peak_times_0 = utils.calculate_peak_times(
            x[:, 0], data["t_seq"], smoothing_window_width, peak_value_threshold
        )
        peak_times_1 = utils.calculate_peak_times(
            x[:, 1], data["t_seq"], smoothing_window_width, peak_value_threshold
        )
        if not (
