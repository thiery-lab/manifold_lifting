...
parser = argparse.ArgumentParser(
    description="Run autoregressive moving average (ARMA) benchmark model experiment"
)
parser.add_argument(
    "--output-root-dir",
    default="results",
...
output_dir = os.path.join(args.output_root_dir, "arma", dir_name)
...
def neg_log_dens_hmc(u):
    return onp.asarray(_neg_log_dens_hmc(u))


def grad_neg_log_dens_hmc(u):
    val, grad = _grad_neg_log_dens_hmc(u)
...
summary_vars = ["μ", "ϕ", "θ", "σ", "hamiltonian"]
